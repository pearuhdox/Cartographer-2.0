...
execute if score @s ca.attr_airdash_count_total matches 1.. unless predicate cartographer_core:world/in_water if score @s ca.airdash_count < @s ca.attr_airdash_count_total unless score @s ca.movement_tech_cdl matches 1.. if block ~ ~-0.2 ~ #cartographer_core:can_raycast unless block ~ ~ ~ #minecraft:climbable unless entity @s[nbt={OnGround:1b}] run function cartographer_custom_attributes:custom_attributes/effects/airdash/tick
