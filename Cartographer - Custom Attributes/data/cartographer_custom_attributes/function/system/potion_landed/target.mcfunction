...
$execute if score $pot_land_damage ca.attr_apply_var matches 1.. as @e[distance=..$(radius),type=!marker,type=!potion,tag=!ca.potion_owner] at @s run function cartographer_custom_attributes:system/potion_landed/damage with storage cartographer:custom_attributes active_projectile
