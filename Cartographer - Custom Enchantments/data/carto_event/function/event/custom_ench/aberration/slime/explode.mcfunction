...
$execute if entity @s[tag=ca.player_spawned] positioned ~-$(half_size) ~-1 ~-$(half_size) as @e[type=#cartographer_core:affected_by_carto,limit=5,sort=nearest,dx=$(size_neg_1),dy=1,dz=$(size_neg_1)] positioned ~$(half_size) ~1 ~$(half_size) run function carto_event:event/custom_ench/aberration/slime/damage with storage carto_event current[-1].parameters
