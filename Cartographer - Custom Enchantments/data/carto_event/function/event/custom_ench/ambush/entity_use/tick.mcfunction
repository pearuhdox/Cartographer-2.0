...
$execute as $(target) at @s unless score @s ca.ambush_time matches 71.. run scoreboard players add @s ca.ambush_time 2
$execute as $(target) at @s if score @s ca.ambush_time matches 1..70 run function carto_event:event/custom_ench/ambush/charge_vfx_entity

$execute as $(target) at @s if score duration= carto_event matches 2.. run attribute @s minecraft:movement_speed modifier add ca.ambush_slow_mob -0.35 add_multiplied_total
$execute as $(target) at @s if score duration= carto_event matches 1 run attribute @s minecraft:movement_speed modifier remove ca.ambush_slow_mob
...
$execute if score duration= carto_event matches 21 as $(telegraph) at @s run function carto_event:event/custom_ench/ambush/entity_use/telegraph_animation

$execute if score duration= carto_event matches 3..21 as $(telegraph) at @s as @a[distance=..$(half_size)] run function carto_event:event/custom_ench/ambush/entity_use/warning

$execute if score duration= carto_event matches 2..85 as $(telegraph) at @s run tp @s ~ ~ ~ ~8 ~

$execute if score duration= carto_event matches 2.. as $(target) at @s run tag @s add ca.is_ambush
...
$execute if score duration= carto_event matches 1 as $(target) at @s run function carto_event:event/custom_ench/ambush/entity_use/appear_vfx with storage carto_event current[-1].parameters

$execute if score duration= carto_event matches 1 as $(target) at @s positioned ~-$(half_size) ~ ~-$(half_size) as @a[dx=$(size_neg_1),dy=1,dz=$(size_neg_1)] positioned ~$(half_size) ~ ~$(half_size) if entity @s[distance=..$(half_size)] run function carto_event:event/custom_ench/ambush/entity_use/damage with storage carto_event current[-1].parameters
...
$execute if score duration= carto_event matches 1 as $(target) at @s run tag @s remove ca.is_ambush
$execute if score duration= carto_event matches 1 as $(target) at @s run scoreboard players set @s ca.ambush_time 0
