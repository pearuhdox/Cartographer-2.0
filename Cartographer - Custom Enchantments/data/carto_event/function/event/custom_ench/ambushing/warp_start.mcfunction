...
execute if entity @s[type=player] anchored eyes positioned ^ ^-0.3 ^ summon item_display run function carto_event:event/custom_ench/ambushing/warp_player_branch with storage carto_event current[-1].parameters
execute if entity @s[type=player] run function carto_event:event/custom_ench/ambushing/warp_player_macro with storage carto_event current[-1].parameters