...
$execute as $(target) at @s if score duration= carto_event matches 1 if score $total_ct ca.stuck_count matches 1.. run function carto_event:api/create_single_entity_event {event:"custom_ench/barbed",duration:37,delay:003,parameters:{},merge_behavior:"merge"}
