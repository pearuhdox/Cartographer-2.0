...
scoreboard players operation $mod ca.ench_starfall_lvl %= $10 ca.CONSTANT
...
$execute as $(target) positioned $(x) $(y) $(z) positioned ~ ~0.1 ~ if score $mod ca.ench_starfall_lvl matches 9 run function carto_event:event/custom_ench/starfall/meteor/vfx with storage carto_event current[-1].parameters
...
$execute if score duration= carto_event matches 1 as $(target) positioned $(x) $(y) $(z) run playsound minecraft:entity.generic.explode ambient @a ~ ~ ~ 1 2
...
$execute if score duration= carto_event matches 21.. as $(target) positioned $(x) $(y) $(z) if entity @s[type=player] positioned ~-$(half_size) ~ ~-$(half_size) as @e[type=#bb:hostile,tag=!ca.trailblazer_cdl,dx=$(size_neg_1),dy=0,dz=$(size_neg_1)] at @s run effect give @s slowness 1 0

$execute if score duration= carto_event matches 1..5 as $(target) positioned $(x) $(y) $(z) if entity @s[type=player] positioned ~-$(half_size) ~ ~-$(half_size) as @e[type=#bb:hostile,limit=2,tag=!ca.starfall_cdl,dx=$(size_neg_1),dy=0,dz=$(size_neg_1)] at @s run function carto_event:event/custom_ench/starfall/meteor/damage with storage carto_event current[-1].parameters
