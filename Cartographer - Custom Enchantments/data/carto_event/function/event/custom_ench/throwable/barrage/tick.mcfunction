...
$execute as $(target) at @s if score $time ca.ench_barrage_lvl >= $mod ca.ench_barrage_lvl run function carto_event:event/custom_ench/throwable/throw/branch with storage carto_event current[-1].parameters
