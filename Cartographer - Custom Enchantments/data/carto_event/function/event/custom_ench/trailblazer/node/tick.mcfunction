...
$execute as $(target) positioned $(x) $(y) $(z) if entity @s[type=player] positioned ~-$(half_size) ~ ~-$(half_size) as @e[type=#bb:hostile,tag=!ca.trailblazer_cdl,dx=$(size_neg_1),dy=0,dz=$(size_neg_1)] at @s run function carto_event:event/custom_ench/trailblazer/node/damage with storage carto_event current[-1].parameters
