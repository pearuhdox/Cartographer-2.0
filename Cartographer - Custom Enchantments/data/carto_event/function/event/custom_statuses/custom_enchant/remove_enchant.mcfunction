...
$execute store result storage cartographer:custom_statuses custom_ench.item.components.minecraft:enchantments."$(enchant_id)" int 1 run scoreboard players remove $ench_value_body ca.ench_value $(level)
$execute if score $ench_value_body ca.ench_value matches 0 run data remove storage cartographer:custom_statuses custom_ench.item.components.minecraft:enchantments."$(enchant_id)"
