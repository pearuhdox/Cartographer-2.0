...
scoreboard players set @s ca.ench_riposte_gear_lvl 0

scoreboard players set $ench ca.ench_var 0
execute store result score $ench ca.ench_var run data get storage cartographer_core:player_equip data.offh.components.minecraft:enchantments.levels.cartographer_custom_enchantments:weapon/unique/melee/riposte
scoreboard players operation @s ca.ench_riposte_gear_lvl += $ench ca.ench_var
