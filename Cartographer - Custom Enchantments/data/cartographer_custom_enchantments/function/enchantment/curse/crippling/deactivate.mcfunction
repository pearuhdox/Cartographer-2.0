...
execute if entity @s[tag=ca.crippling_activated] if entity @s[type=player] run playsound minecraft:block.ender_chest.close player @s ~ ~ ~ 0.25 1.8
execute if entity @s[tag=ca.crippling_activated] unless entity @s[type=player] run playsound minecraft:block.ender_chest.close hostile @a[distance=..16] ~ ~ ~ 0.5 1.8
tag @s remove ca.crippling_activated