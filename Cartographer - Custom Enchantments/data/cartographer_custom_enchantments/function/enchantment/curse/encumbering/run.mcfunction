...
execute if entity @s[type=player] if score @s ca.ench_encumbering_lvl matches 1.. unless predicate bb:cant_crit run attribute @s attack_damage modifier add ca.encumbering_penalty -0.33 add_multiplied_total
execute if entity @s[type=player] if score @s ca.ench_encumbering_lvl matches 1.. if predicate bb:cant_crit run attribute @s attack_damage modifier remove ca.encumbering_penalty