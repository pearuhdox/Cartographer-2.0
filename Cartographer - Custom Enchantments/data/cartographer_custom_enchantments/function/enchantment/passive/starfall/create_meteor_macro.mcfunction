...
$function carto_event:api/create_single_entity_event {event:"custom_ench/starfall/meteor",duration:$(delay),delay:000,parameters:{telegraph:"$(telegraph)",meteor_height:$(delay),x:$(x),y:$(y),z:$(z),block:"$(block)",size:$(size),half_size:$(half_size),size_neg_1:$(size_neg_1),damage:$(damage)},merge_behavior:"none"}
