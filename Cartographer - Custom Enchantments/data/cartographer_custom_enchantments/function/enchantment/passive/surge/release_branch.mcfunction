...
$execute if entity @s[type=player] positioned ~-$(half_size) ~ ~-$(half_size) as @e[type=#bb:hostile,limit=5,tag=!ca.surge_cdl,dx=$(size_neg_1),dy=3,dz=$(size_neg_1)] at @s run function cartographer_custom_enchantments:enchantment/passive/surge/damage with storage cartographer:custom_enchantments
