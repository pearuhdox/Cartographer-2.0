...
execute unless score @s ca.recoil_cooldown matches 1.. unless entity @s[tag=ca.has_custom_status_grounded] unless predicate cartographer_core:player/sneaking at @s run tp @s @s

execute unless score @s ca.recoil_cooldown matches 1.. unless entity @s[tag=ca.has_custom_status_grounded] if entity @s[type=player] unless predicate cartographer_core:player/sneaking at @s anchored feet positioned ^ ^ ^ facing ^ ^ ^-1 rotated ~ ~-20 run function cartographer_custom_enchantments:enchantment/weapon/unique/general/recoil/do_push_player with storage cartographer:custom_enchantments
