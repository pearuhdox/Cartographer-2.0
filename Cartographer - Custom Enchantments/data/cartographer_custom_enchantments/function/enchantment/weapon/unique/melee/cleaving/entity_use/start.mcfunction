...
execute store result score $windup_item ca.ench_cleaving_lvl run data get entity @s equipment.mainhand.components.minecraft:attribute_modifiers[{id:"minecraft:attack_speed"}].amount 100
execute if score $windup_item ca.ench_cleaving_lvl matches 0 run scoreboard players set $windup_item ca.ench_cleaving_lvl -240
scoreboard players add $windup_item ca.ench_cleaving_lvl 400
scoreboard players set $windup_value ca.ench_cleaving_lvl 4800
scoreboard players operation $windup_value ca.ench_cleaving_lvl /= $windup_item ca.ench_cleaving_lvl
execute if score $windup_value ca.ench_cleaving_lvl matches 0 run scoreboard players set $windup_value ca.ench_cleaving_lvl 30
execute if score $windup_value ca.ench_cleaving_lvl matches ..15 run scoreboard players set $windup_value ca.ench_cleaving_lvl 16
execute store result storage cartographer:custom_enchantments windup int 1 run scoreboard players get $windup_value ca.ench_cleaving_lvl
