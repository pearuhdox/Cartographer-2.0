...
execute unless entity @s[tag=ca.has_custom_status_grounded] unless entity @s[tag=ca.has_custom_status_silenced] unless entity @s[tag=ca.is_lunging] run function carto_event:api/create_single_entity_event {event:"custom_ench/lunging",duration:5,delay:000,parameters:{},merge_behavior:"merge"}
