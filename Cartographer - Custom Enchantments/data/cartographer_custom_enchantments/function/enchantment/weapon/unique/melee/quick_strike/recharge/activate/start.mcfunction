scoreboard players remove $time_current ca.ench_quick_strike_slot 1
execute unless score $time_current ca.ench_quick_strike_slot matches 1.. run function cartographer_custom_enchantments:enchantment/weapon/unique/melee/quick_strike/recharge/activate/refill/start

execute unless score $time_current ca.ench_quick_strike_slot matches 1.. run scoreboard players set $time_current ca.ench_quick_strike_slot 0

execute store result storage cartographer_custom_enchantments:quick_strike items[0].components.minecraft:custom_data.quick_strike_cooldown int 1 run scoreboard players get $time_current ca.ench_quick_strike_slot
...
execute unless data storage cartographer_custom_enchantments:quick_strike items[0].components.minecraft:custom_data.quick_strike_name run function cartographer_custom_enchantments:enchantment/weapon/unique/melee/quick_strike/recharge/activate/visual/choose_default_name
...
execute store result storage cartographer_custom_enchantments:quick_strike visual.time int 1 run scoreboard players get $time_current ca.ench_quick_strike_slot
