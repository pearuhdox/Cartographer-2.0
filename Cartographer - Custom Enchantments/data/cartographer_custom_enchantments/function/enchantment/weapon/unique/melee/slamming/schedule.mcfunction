...
execute unless entity @s[type=player] unless entity @s[tag=has_custom_status_silenced] if predicate cartographer_core:periodic_tick/5 if score @s ca.ench_slamming_lvl matches 1.. unless entity @s[tag=ca.is_slamming] if entity @a[distance=..5,gamemode=!spectator,gamemode=!creative] run function cartographer_custom_enchantments:enchantment/weapon/unique/melee/slamming/entity_use/start