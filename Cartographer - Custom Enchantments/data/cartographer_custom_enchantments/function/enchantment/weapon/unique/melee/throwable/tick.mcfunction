...
execute unless entity @s[tag=ca.has_custom_status_silenced] unless score @s ca.throwable_cooldown matches 1.. run function carto_event:api/create_single_entity_event {event:"custom_ench/throwable",duration:5,delay:000,parameters:{},merge_behavior:"merge"}
