tag @s remove ca.end_thrusting

execute summon text_display run function cartographer_custom_enchantments:enchantment/weapon/unique/melee/thrusting/entity_use/create_telegraph
...
scoreboard players operation $half_size ca.ench_thrusting_lvl = $size ca.ench_thrusting_lvl
scoreboard players operation $size_neg_1 ca.ench_thrusting_lvl = $size ca.ench_thrusting_lvl

scoreboard players remove $size_neg_1 ca.ench_thrusting_lvl 100
scoreboard players operation $half_size ca.ench_thrusting_lvl /= $2 ca.CONSTANT

execute store result storage cartographer:custom_enchantments size_neg_1 double 0.01 run scoreboard players get $size_neg_1 ca.ench_thrusting_lvl
execute store result storage cartographer:custom_enchantments half_size double 0.01 run scoreboard players get $half_size ca.ench_thrusting_lvl
