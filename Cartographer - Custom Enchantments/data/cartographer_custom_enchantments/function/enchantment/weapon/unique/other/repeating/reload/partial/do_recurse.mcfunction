...
execute store result storage cartographer_custom_enchantments:repeating data.item.components.minecraft:custom_data.repeating_ammo int 1 run scoreboard players add $current_ammo_amount ca.ench_repeating_lvl 1


scoreboard players remove $ammo_reload_per ca.ench_repeating_lvl 1
execute if score $ammo_reload_per ca.ench_repeating_lvl matches 1.. if score $current_ammo_amount ca.ench_repeating_lvl < $allowed_max_ammo ca.ench_repeating_lvl if data storage cartographer_custom_enchantments:repeating data.arrows[0] run function cartographer_custom_enchantments:enchantment/weapon/unique/other/repeating/reload/partial/do_recurse