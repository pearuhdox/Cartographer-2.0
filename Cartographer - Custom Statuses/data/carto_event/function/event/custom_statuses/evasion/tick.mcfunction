$execute as $(target) at @s positioned ~ ~1 ~ if predicate cartographer_core:periodic_tick/4 run particle entity_effect{color:[0.820,0.965,1.000,1.00]} ~ ~ ~ 0.25 0.25 0.25 1 1 normal
...
$execute as $(target) at @s if score duration= carto_event matches 2.. unless entity @s[tag=ca.has_custom_status_evasion] run function carto_event:event/custom_statuses/evasion/attempt

$execute as $(target) at @s run tag @s remove ca.has_custom_status_evasion
$execute as $(target) at @s if score duration= carto_event matches 2.. run tag @s add ca.has_custom_status_evasion
...
$execute as $(target) at @s if entity @s[tag=ca.purged] run tag @s remove ca.has_custom_status_evasion
