execute store result score $status_damage ca.status_var run data get storage carto_event current[-1].parameters.damage 100
...
execute store result storage carto_event current[-1].parameters.damage double 0.01 run scoreboard players get $status_damage ca.status_var