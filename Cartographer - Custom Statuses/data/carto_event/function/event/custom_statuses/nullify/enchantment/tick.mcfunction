...
$execute as $(target) at @s if score duration= carto_event matches 25.. unless entity @s[tag=ca.is_parrying] if score @s ca.nullify_cooldown matches 1 positioned ~ ~0.2 ~ run function carto_event:event/custom_statuses/nullify/vfx
$execute as $(target) at @s if score duration= carto_event matches 25.. unless entity @s[tag=ca.is_parrying] if entity @s[type=!player] if score @s ca.nullify_cooldown matches 1 positioned ~ ~1.2 ~ run function carto_event:event/custom_statuses/nullify/vfx


$execute as $(target) at @s if score duration= carto_event matches 2.. unless entity @s[tag=ca.is_parrying] unless score @s ca.nullify_cooldown matches 1.. if entity @s[nbt={HurtTime:9s}] run function carto_event:event/custom_statuses/nullify/use with storage carto_event current[-1].parameters
