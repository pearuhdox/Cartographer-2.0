$execute as $(target) at @s positioned ~ ~1 ~ unless entity @s[type=player] if predicate cartographer_core:periodic_tick/4 run particle entity_effect{color:[0.804,0.361,0.671,1.00]} ~ ~ ~ 0.2 0.2 0.2 1 1 normal

$execute as $(target) at @s run tag @s remove ca.has_custom_status_regeneration_ench
$execute as $(target) at @s if score duration= carto_event matches 2.. run tag @s add ca.has_custom_status_regeneration_ench
