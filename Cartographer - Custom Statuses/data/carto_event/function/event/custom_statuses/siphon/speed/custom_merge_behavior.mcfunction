...
execute if score $effect_old_amt ca.merge_var < $effect_new_amt ca.merge_var store result storage cartographer_custom_statuses:siphon merge.speed double 0.01 run scoreboard players get $effect_new_amt ca.merge_var
...
execute if score $effect_old_amt ca.merge_var < $effect_new_amt ca.merge_var run function carto_event:event/custom_statuses/siphon/speed/custom_merge_player with storage cartographer_custom_statuses:siphon merge
