...
$execute as $(old_player) at @s if score @s ca.siphon_player_test matches 2.. run scoreboard players operation @s ca.siphon_strength_player -= $effect_old_amt ca.status_var
$scoreboard players operation $(new_player) ca.siphon_strength_player += $effect_new_amt ca.status_var