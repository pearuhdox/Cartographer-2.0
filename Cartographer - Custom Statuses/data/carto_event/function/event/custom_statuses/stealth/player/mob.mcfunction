...
execute if score $stealth_target ca.status_var matches 1.. run function carto_event:api/create_single_entity_event {event:"custom_statuses/vanilla_attribute",duration:5,delay:000,parameters:{type:"minecraft:follow_range",tag:"ca.stealth_mob_blindness_attr",id:"ca.stealth_blindness_mob",value:-100,operation:"add_value",tick_command:""},merge_behavior:"none"} 