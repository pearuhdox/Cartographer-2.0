...
execute on attacker if entity @s[tag=ca.apply_status_melee_hit_self] run function cartographer_custom_statuses:apply_status/apply/start_loop {location:"bbl:pldata",path:"sudo_root.working_data.cartographer.apply_status_data.actions",action:"melee_hit",type:"self"}
#Write to player data
execute on attacker if entity @s[tag=ca.apply_status_melee_hit_self] run function bb:lib/pldata/write
