...
execute unless entity @s[type=player] unless entity @s[tag=ca.ench_ran_flag_shielding_hit] run function carto_event:api/create_single_entity_event {event:"schedule_enchant/reset",duration:1,delay:0,parameters:{flag:"shielding_hit"},merge_behavior:"merge"}
